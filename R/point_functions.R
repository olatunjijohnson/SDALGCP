...
##' lambdamax <- 6835
##' delta <- 0.1
##' pop_shp <- load('population_density.RData') # raster file of population density
##' xy <- Rej_SSIP(poly, delta, pop_shp, lambdamax, rho=0.55, giveup=1000)
##' }
##' @author Olatunji O. Johnson \email{o.johnson@@lancaster.ac.uk}
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
...
##' \donttest{
##' load(popshape.RData)
##' xy <- gridPointsSSIP(shp=popshape, delta, rho=0.55, giveup=1000, divisor=1)
##' pre.matrix <- precompute.corr.matrix(S.coord=xy, phi=c(1:5))
##' }
##' @author Olatunji O. Johnson \email{o.johnson@@lancaster.ac.uk}
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
...
##' m <- rpois(n, lambda=20)
##' fm <- y ~ offset(log(m)) + x
##' model <- glm(fm, family="poisson")
##' beta.start <-coef(model)
##' sigma2.start <- mean(model$residuals^2)
##' h <- 1.65/(n^(1/6))
##' control.mcmc <- list(n.sim = 10000, burnin = 2000, thin= 80, h=h, c1.h = 0.01, c2.h = 1e-04)
##' par0 <- c( beta.start, sigma2.start)
##' corr <- matrix(data = runif(n^2), nrow =n, ncol = n)
##' diag(corr) <- 1
##' result <- Aggregated_poisson_log_MCML(y, D, m, corr= corr, par0=par0, control.mcmc=control.mcmc)
...
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
##' @references Giorgi, E., & Diggle, P. J. (2017). PrevMap: an R package for prevalence mapping. Journal of Statis.
...
Aggregated_poisson_log_MCML <- function(y, D, m, corr, par0, control.mcmc) {
  ...
  # S.sim.res <- PrevMap::Laplace.sampling(mu0, Sigma0, y, m, control.mcmc,
...
##' \donttest{
##' h <- 1.65/(n^(1/6))
##' control.mcmc <- list(n.sim = 10000, burnin = 2000, thin= 8, h=h, c1.h = 0.01, c2.h = 1e-04)
...
##' @examples
##' \donttest{
##' load('popshape_coords.RData')
##' load('popshape.pre.compute.RData')
##' load('popshape_outputnewlong.RData')
##' rrr <- SDAContinuousPred(my.shp=popshape, popshape.pre.compute, 
##' new.res, grid.point.data, divisor=1000)
##' }
##' @author Olatunji O. Johnson \email{o.johnson@@lancaster.ac.uk}
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
...
##' load('popshape.pre.compute.RData')
##' load('popshape_outputnewlong.RData')
##' rrr <- SDAContinuousPred(my.shp=popshape, popshape.pre.compute, new.res,
##' grid.point.data, divisor=1000)
##' }
##' @author Olatunji O. Johnson \email{o.johnson@@lancaster.ac.uk}
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
...
SDAContinuousPred <- function(para_est, cellsize, control.mcmc=NULL, pred.loc=NULL,
  ...
  cov.matrix.x.A3=function(pred.loc, S.coord, phi){
    ...
    tmp3 <- foreach::foreach(i = 1:n.pred.loc, .combine = 'c') %:%
      foreach::foreach(j = 1:n.distr, .combine = 'c') %dopar% {
        U <- as.matrix(pdist::pdist(pred.loc[i,],
                                    as.matrix(S.coord[[j]]$xy)))
        R[i,j] <-  mean(exp(-U/phi))
        NULL
...
##' @param obj an object of class 'SDALGCP' obtained as result of a call to \link{SDALGCPPred}
...
##' @description This function plots the region specific incidence and covariate adjusted relative risk and their corresponding standard error using the output from \code{\link{SDALGCPPred}} function.
##' @param obj an object of class 'SDALGCP' obtained as result of a call to \code{\link{SDALGCPPred}}
...
##' @description Computes the exceedance probability for a given threshold of the spatially continuous relative risk or the region specific relative risk from the object of class \code{\link{SDALGCP}}.
##' @param obj an object of class 'SDALGCP' obtained as result of a call to \link{SDALGCPPred}.
...
##' @description Plot the exceedance probability map for a given threshold of the spatially continuous relative risk or the region specific relative risk from the object of class \code{\link{SDALGCP}}.
##' @param obj an object of class 'SDALGCP' obtained as result of a call to \link{SDALGCPPred}.
##' @param thresholds either a vector  of numbers or a vector of single value.
...
###############################
##' @title plot.SDALGCP function
##' @description Simple plotting function for both discrete and conituous prediction from the object of class \link{Pred.SDALGCP}.
##' @param obj an object of class 'Pred.SDALGCP' obtained as result of a call to \link{SDALGCPPred}.
##' @param type Character string: what type of plot to produce. For discrete inference choices are "incidence" (=exp(mu+S)); "SDincidence" (standard error of incidence); "CovAdjRelRisk" (=exp(S)); or "SDCovAdjRelRisk" (standard error of covariate adjusted relative risk); while for continuous inference, choices are "relrisk" (=exp(S)); "SErelrisk" (standard error of the relative risk).
##' @param thresholds either a vector  of numbers or a vector of single value.
##' @param continuous logical; TRUE for spatially continuous relative risk and FALSE for region specific relative risk. default is TRUE
##' @param bound optional; it gives the boundary of the region, only useful when the predictive location is supplied in \link{SDALGCPPred}
##' @param ... further arguments passed to \link{plot}.
##' @seealso \link{SDALGCPPred}, \link{plot_continuous}, \link{plot_discrete}, \link{plot_SDALGCPexceedance},  \link{SDALGCPPred}
##' @return The function does not return any value.
##' @author Olatunji O. Johnson \email{o.johnson@@lancaster.ac.uk}
##' @author Emanuele Giorgi \email{e.giorgi@@lancaster.ac.uk}
##' @author Peter J. Diggle \email{p.diggle@@lancaster.ac.uk}
plot.Pred.SDALGCP <- function(obj,  type='relrisk', continuous=TRUE, thresholds=NULL, bound=NULL, ...){
  if(continuous){
    if(is.null(thresholds)){
      plot_continuous(obj=obj, bound=bound, type=type, ...)
    } else {
      plot_SDALGCPexceedance(obj=obj , thresholds=thresholds, bound=bound, continuous=continuous, ...)
    }
  }else{
    if (is.null(thresholds)){
...
##' @description Computes confidence intervals for one or more parameters in a fitted SDALGCP model from the object of class \code{\link{SDALGCP}}, based on asymptotic normality.
##' @param object an object of class 'SDALGCP' obtained as result of a call to \link{SDALGCPMCML}.
##' @param parm a specification of which parameters are to be given confidence intervals, either a vector of numbers or a vector of names. If missing, all parameters are considered.
##' @param level the confidence level required.
##' @param dp the number of decimal places for the result
##' @param ... additional argument(s) for methods.
##' @seealso \link{confint.lm}, \link{confint.default}, \link{SDALGCP}, \link{SDALGCPMCML}
